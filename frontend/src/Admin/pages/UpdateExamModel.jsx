    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fadeIn">
            ...
            <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl p-6 relative animate-slideUp overflow-y-auto max-h-[90vh]">
                ...
                {/* CLOSE BUTTON */}
                <button onClick={closeModal} className="absolute top-4 right-4 p-2 rounded-full bg-gray-200">
                    <X className="w-5 h-5" />
                </button>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-10">
                    ...
                    <div className="p-6 bg-indigo-50 rounded-xl border">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <input {...register("ExamName")} placeholder="Exam Name" className="border p-3 rounded w-full" />
                            <input {...register("ExamCode")} placeholder="Exam Code" className="border p-3 rounded w-full" />
                            <input {...register("Class")} placeholder="Class" className="border p-3 rounded w-full" />
                            <input {...register("Duration", { valueAsNumber: true })} type="number" placeholder="Duration (Minutes)" className="border p-3 rounded w-full" />
                            <input {...register("MarkPerQuestion", { valueAsNumber: true })} type="number" placeholder="Marks Per Question" className="border p-3 rounded w-full" />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        {fields.map((item, index) => (
                            <div key={item.id} className="p-4 border rounded mb-4 relative">
                                ...
                                <button type="button" onClick={() => remove(index)} className="absolute top-0 -right-6 text-red-600">
                                    <Trash2 />
                                </button>
                        ...
                        <button
                            type="button"
                            onClick={() => append({ Name: "", Options: ["", "", "", ""], Answer: "" })}
